...
playday = datetime.date(2021, 10,1)
...
def hero():
    ...
    def chalo():
        ...
        def animate():
            for c in itertools.cycle(['|', '/', '-', '\\']) :
                if done:
                    break
                sys.stdout.write('\rhacking in the emard server for next colour--------- ' + c)
                sys.stdout.flush()
                time.sleep(0.1)
    ...
    while(y):
        clear()
        system(banner)
        print("Weekly members play PARITY all other play EMARD")
        print("Contact me on telegram @HACKMGK")
        print("Enter ",newperiod," EMARD Price :")
        current=input()
        current=int(current)
        chalo()
